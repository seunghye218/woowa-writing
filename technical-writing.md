...
더 넓은 개념으로, 인터넷 상의 모든 자원을 식별하는 데 사용되는 고유한 이름입니다.
자원의 위치뿐만 아니라, 자원에 대한 다양한 정보를 포함할 수 있습니다.
예시: mailto:[이메일 주소 삭제됨], tel:+1-800-555-1212
...
**HSTS(HTTP Strict Transport Security)**는 웹사이트가 HTTPS 연결만을 강제하도록 브라우저에 지시하는 보안 메커니즘입니다. 즉, HSTS가 설정된 웹사이트에 접속하려 할 때, 브라우저는 자동으로 HTTP 대신 HTTPS를 사용하여 연결을 시도합니다. 이를 통해 프로토콜 다운그레이드 공격과 같은 보안 위협으로부터 웹사이트를 보호할 수 있습니다.

- HSTS가 필요한 이유

프로토콜 다운그레이드 공격 방지: 공격자가 HTTP로 연결을 강제하여 보안이 취약한 환경에서 통신을 유도하는 공격을 방지합니다.
쿠키 하이재킹 방지: HTTP로 전송되는 쿠키는 쉽게 가로챌 수 있지만, HTTPS로 전송되는 쿠키는 암호화되어 있어 안전합니다.
사용자 신뢰도 향상: HTTPS 연결은 웹사이트의 신뢰도를 높이는 데 기여합니다.
...
- 부하 분산
단일 서버의 한계로 하나의 서버로 많은 트래픽을 처리하면 서버에 과부하가 걸려 서비스가 느려지거나 중단될 수 있습니다.
이때 여러 서버 분산로 동일한 웹 서비스를 여러 대의 서버에 분산 배치하면 각 서버가 일부 트래픽만 처리하기 때문에 서버 부하를 줄이고 서비스 속도를 향상시킬 수 있습니다.

- 서비스 안정성
단일 장애지점으로 하나의 서버에 문제가 발생하면 전체 서비스가 중단될 수 있습니다.
여러 서버에 서비스를 분산 배치하면 한 개의 서버에 문제가 발생하더라도 다른 서버에서 서비스를 계속 제공할 수 있어 서비스 안정성을 높일 수 있습니다.
- 확장성

트래픽 증가에 유연하게 대응할 수 있습니다. 서비스 이용자가 늘어나 트래픽이 증가하더라도, 추가 서버를 쉽게 확장하여 처리 능력을 늘릴 수 있습니다.
...
CSR은 웹 페이지를 클라이언트(브라우저)에서 렌더링하는 방식입니다. 초기에는 HTML, CSS, JavaScript 파일만 전송하고, 브라우저에서 JavaScript를 실행하여 동적인 페이지를 생성합니다.

- 장점
빠른 상호 작용: 사용자의 입력에 따라 페이지가 실시간으로 업데이트되어 사용자 경험이 좋습니다.
단일 페이지 애플리케이션(SPA) 구축: SPA를 구축하기에 적합하여 복잡한 웹 애플리케이션 개발에 유리합니다.

- 단점
초기 로딩 시간: 모든 JavaScript 파일을 다운로드하고 실행해야 하므로 초기 로딩 시간이 길어질 수 있습니다.
SEO: 검색 엔진이 JavaScript를 완벽하게 해석하지 못할 수 있어 SEO에 불리할 수 있습니다.


### SSR(Server-Side Rendering)

SSR은 웹 페이지를 서버에서 미리 렌더링하여 완성된 HTML을 클라이언트에 전송하는 방식입니다.

- 장점
빠른 초기 로딩: 완성된 HTML을 전송하므로 브라우저가 바로 페이지를 표시할 수 있습니다.
SEO: 검색 엔진이 HTML을 완벽하게 해석할 수 있어 SEO에 유리합니다.
소셜 미디어 공유: 소셜 미디어에서 정확한 미리보기를 제공할 수 있습니다.

- 단점
서버 부하 증가: 모든 요청에 대해 서버에서 HTML을 생성해야 하므로 서버 부하가 증가할 수 있습니다.
동적 업데이트: 클라이언트에서 발생하는 동적 업데이트는 JavaScript를 통해 처리해야 합니다.

### CSR과 SSR의 조합

최근에는 CSR과 SSR의 장점을 모두 살린 하이브리드 방식이 많이 사용됩니다. 초기에는 SSR을 통해 완성된 HTML을 전송하고, 이후에는 JavaScript를 통해 동적인 업데이트를 처리하는 방식입니다. 이를 통해 초기 로딩 속도를 빠르게 유지하면서 동적인 사용자 경험을 제공할 수 있습니다.
...
### 렌더링 최적화
미네화: HTML, CSS, JavaScript 파일의 크기를 줄여 전송 시간을 단축합니다.
